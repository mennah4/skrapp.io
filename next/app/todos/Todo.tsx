...
export default function Todo({ todo, updateStatus, updateText, deleteTodo }: { todo: any, updateStatus: any, updateText: any, deleteTodo: any }) {
    const [text, setText] = useState(todo.text);
    const textFormRef = createRef<HTMLFormElement>();
    const doneFormRef = createRef<HTMLFormElement>();


    return (
        <div className="relative">
            <div className="w-full flex items-center" >
                <form action={updateStatus} ref={doneFormRef}>
                    <input type="hidden" name="id" value={todo.id} />
                    <input type="hidden" name="done" value={String(!todo.done)} />
                    <Checkbox isSelected={todo.done} color="success" onChange={() => doneFormRef.current?.requestSubmit()}></Checkbox>
                </form>
                <form action={updateText} ref={textFormRef}
                    className="flex-1">
                    <input type="hidden" name="id" value={todo.id} />
                    <Input
                        isRequired
                ...
                <div className="space-x-2 absolute top-1/2 right-10 -translate-y-1/2 flex items-center z-10">
                    {todo.initialText === text ? <></> :
                        <Button color="primary" variant="ghost" className="rounded-full min-w-6 h-6 p-0" type="submit"
                            onClick={() => {
                    ...
                    <form action={deleteTodo} className="flex items-center">
                        <input type="hidden" name="id" value={todo.id} />
                        <Button color="danger" variant="ghost" className="rounded-full min-w-6 h-6 p-0" type="submit">
                            <svg
                                viewBox="0 0 24 24"
                                fill="currentColor"
                                xmlns="http://www.w3.org/2000/svg"
            ...
            <Link href={`/todos/${todo.id}`} className="w-6 h-6 ml-2">
                <svg
                    fill="currentColor"
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
